...
<div class="zh-swiper">
    ...
    <div v-else class="zh-swiper__track">
        <slot/>
    </div>
    <div class="zh-swiper__indicators" v-if="showIndicators && count > 1">
        <i  v-for="index in count"
            :class="{ 'zh-swiper__indicator--active': index - 1 === activeIndicator }">
        </i>
    </div>
</div>
</template>
